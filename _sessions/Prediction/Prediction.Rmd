...
<ul>
  ...
  <ul class="level">
    ...
    <li><span>2 - <high>Repeat</high> until pre-defined threshold (e.g., `minsplit`) splits are no longer possible.</span></li><br>
...
<ul>
  <li class="m1"><span>To <high>test model predictions</high> with `caret`, all you need to do is get a vector of predictions from a new dataframe `newdata` using the `predict()` function.</span></li>
</ul>
